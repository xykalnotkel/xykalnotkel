...
def make_base():
    ...
    # Top label — cleaner, no fluff
    d.rounded_rectangle((tx, 76, tx + 290, 108), radius=16, fill=(79, 52, 118, 190), outline=(168, 144, 211, 75), width=1)
    ...
    # Stack line — clear and specific
    d.text((tx + 2, 188), "FLUTTER  /  RUST  /  WEBRTC  /  CLOUD  /  AI", font=small, fill=PURPLE_SOFT)

    # Tagline — sharp and meaningful
    ...
    d.text((tx + 39, 249), "Building sharp tools for real problems.", font=sub_font, fill=MUTED)

    # URL
    d.text((tx + 1, 324), "github.com/xykalnotkel", font=small, fill=(175, 168, 188))

    # Restrained purple baseline
...
# GIF fallback (for GitHub clients that don't support animated WebP)
frames = []
count = 32
...
# Animated WebP — primary format (smaller, sharper)
try:
    webp_frames = []
    for i in range(count):
        t = i / count
...
print("Generated:")
for name in ["xyspace-header.webp", "xyspace-header.gif", "xyspace-header-animated.webp", "xyspace-avatar.webp", "xyspace-avatar.png", "xyspace-social-preview.webp", "xyspace-social-preview.png"]:
    p = ASSETS / name
    if p.exists():
